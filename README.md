...
- The `mdbx` feature adds a snapshot-backed overlay chainstate on that base: one MDBX environment holds post-base coins, spent-base tombstones, block undo, and the execution tip in four tables, committed per block in one MDBX transaction behind the `ExecutionChainStore` trait that the consensus connect/disconnect executor and `download_execute_batch` pipeline are generic over. The environment carries a hard geometry ceiling (default 3 GiB) that fails commits closed at the budget, and a rebase folds base plus overlay into a fresh self-authenticated compressed snapshot at the current tip, rebuilds the MPHF index, and atomically resets the overlay. Bounded `--snapshot-overlay-catchup SNAPSHOT --snapshot-overlay-index INDEX --once` runs the complete network catch-up on this chainstate through the ordinary peer pool — headers first, then the same staged, prefetch-overlapped execution pipeline with stale-tip disconnection and undo pruning — rebasing automatically at the configured threshold (default 85%).
