...
- Library-first runtime integration: `rbtcd` is a thin command-line adapter over `rbtc::node`. A Tokio host can configure an ordinary persistent node with `NodeBuilder`, retain a cloneable `NodeController`, and await `NodeHandle` as a critical task without installing process signal handlers. This is the first P1 modularization slice for direct `n42-26` integration; per-instance runtime state, typed events, and the GPL-compatible combined-distribution decision remain explicit follow-up gates.
