...
- The `mdbx` feature adds a snapshot-backed overlay chainstate on that base: one MDBX environment holds post-base coins, spent-base tombstones, block undo, and the execution tip in four tables, committed per block in one MDBX transaction behind the new `ExecutionChainStore` trait that the consensus connect/disconnect executor is now generic over. The environment carries a hard geometry ceiling (for example 3 GiB) that fails commits closed at the budget, and a rebase folds base plus overlay into a fresh self-authenticated compressed snapshot at the current tip, rebuilds the MPHF index, and atomically resets the overlay; the live network driver still runs on the default redb chainstate.
