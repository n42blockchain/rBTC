...
| Core cluster optimizer and broader fee-floor parity | **Implementation / differential work remains.** The repository still uses the ancestor-set greedy baseline. Existing replacement/package tests and the new decay test cover specific decisions. | Add bounded optimizer differential workloads and resolve their counterexamples. Compare pressure and decay under equivalent occupancy regimes; rBTC counts retained serialized bytes while Core's threshold uses dynamic memory usage. |
| Whole-pipeline admission CPU and allocation budget | **Implementation remains.** SCRIPT reuse, immutable payload sharing and indexed traversal are measured improvements. | Account for replay overlays, fresh UTXO lookup, hashing, policy checks, metadata copies and snapshots; impose aggregate work/allocation limits across accepted/rejected packages, concurrent peers and chain changes. Preserve contextual validation and atomic admission. |
| Valid competing-header memory/disk limits | **Implementation remains.** Only the extra serving copy is bounded to active ancestors. | Reserve capacity at both peer and local-block ingress; implement resource deferral, resumable stronger-fork recovery, durable eviction and bounded restart loading. Prove retention plateaus without preventing a valid reorg. |
...
| Intermittent failover-test nonce mismatch | **Cause remains unconfirmed.** The first 2026-09-10 admission-index full run failed, while its isolated and complete reruns passed without source changes. | Capture and identify the unexpected connection/nonce under a reproducer. Passing another run does not by itself close this investigation. |
