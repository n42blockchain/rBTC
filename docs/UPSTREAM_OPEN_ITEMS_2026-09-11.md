...
| Whole-pipeline admission CPU and allocation budget | **Partially completed; aggregate limits remain.** SCRIPT reuse, immutable payload sharing and indexed traversal are measured improvements. Reconciliation now performs one contextual validation per entry, retains shared payloads and preserves paid zero-fee parents; see [acceptance](UPSTREAM_RECONCILIATION_GATE.md). | Account for ordinary package replay, fresh UTXO lookup, hashing, policy checks, metadata copies, snapshots and exceptional size-growth checks; impose aggregate work/allocation limits across accepted/rejected packages, concurrent peers and repeated chain changes. Preserve contextual validation and atomic admission. |
