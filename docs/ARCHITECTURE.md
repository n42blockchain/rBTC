...
repeats those constraints inside the execution routine, prints a funds-safety
warning before storage or network startup, and forbids an indefinite node plus
...
The full headers-first/block-execution daemon is currently enabled only for regtest and Signet. Mainnet, legacy testnet, and testnet4 retain protocol/header probing but reject `--data-dir` before a peer connection; the same gate is repeated at the execution entry point. A real default-Signet block fixture exercises handshake, header download, witness-block retrieval, BIP325 validation, atomic UTXO/undo/tip persistence, and pruned-ledger publication without creating a disabled explorer index; the API end-to-end gate separately covers persistent explorer indexing. A wrong custom challenge is rejected without UTXO, undo, or tip residue. This boundary widens only after the remaining network activation and historical-corpus gates pass.
