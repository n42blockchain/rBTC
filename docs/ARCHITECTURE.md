...
Consensus script regression tests vendor Bitcoin Core 26's complete `tx_valid.json` and `tx_invalid.json` files byte-for-byte, parse Core's script-assembly syntax, and execute all 119 valid cases plus the 70 invalid cases expressible through public `libbitcoinconsensus` flags using the same rBTC adapter as block connection. The harness separately accounts for 9 `BADTX` structure cases and 14 policy-only cases instead of conflating either with the public consensus API. Constructed Taproot cases additionally exercise Core 26's spent-output ABI, commitment proof, and tapscript result. Full `script_tests.json` ingestion and live `bitcoin-cli` regtest comparisons remain acceptance gates.
