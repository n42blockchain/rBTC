...
Consensus script regression tests use transactions and expected outcomes pinned to Bitcoin Core 26's `tx_valid.json` and `tx_invalid.json`, then execute them through the same rBTC adapter used during block connection. Constructed Taproot cases additionally exercise Core 26's spent-output ABI, commitment proof, and tapscript result. These representative cases are an initial differential gate, not a substitute for ingesting the complete upstream corpus or running live `bitcoin-cli` regtest comparisons.
