...
- [ ] Complete block/contextual validation: BIP30 is enforced with its two historical exceptions; BIP34 height, BIP34/BIP66/BIP65 minimum header versions, BIP68/113 locks, deployment-aware BIP141 witness commitment/unexpected-witness rules, mutated transaction Merkle trees, buried deployments, Taproot BIP9 state, Core-compatible configurable regtest Taproot activation, network-specific subsidy halving intervals, Core-compatible unspendable-output UTXO pruning, the 80,000 legacy/P2SH/witness sigop-cost limit, and Core-style transaction duplicate/null-input/base-size checks are enforced. The complete pinned Core 26 transaction-vector corpus now runs offline through rBTC's production script adapter: all 119 valid cases and all 70 invalid cases expressible by public consensus flags, with 9 `BADTX` structure cases and 14 policy-only cases classified separately. The harness also parses all 1,207 `script_tests.json` cases and executes the 230 cases whose full flag set is public consensus API, while constructed Core-backed cases cover BIP341 commitments and BIP342 tapscript execution. A live Core 26 regtest gate now compares two sequential valid blocks and excessive-coinbase, BIP34-height, multiple-coinbase, and bad-Merkle rejection through the production connection path. Remaining gates include a broad historical block corpus and keeping policy/standardness distinct from consensus.
- [ ] Differential tests against Bitcoin Core test vectors and `bitcoin-cli`/regtest. Core 26's complete transaction and script JSON files are pinned in normal CI with the public consensus-API boundary made explicit: the script corpus contributes 148 expected passes and 82 expected failures, while 977 policy-only cases remain classified but deliberately unexecuted through `libbitcoinconsensus`. The optional live Core 26 gate additionally proves matching accept/reject outcomes and no rBTC tip/undo/UTXO residue after rejected candidates. Expanding that gate across historical activation boundaries, property tests, cargo-fuzz, and CI-provisioned Core remain.
