...
- [ ] Complete block/contextual validation: BIP30 is enforced with its two historical exceptions; BIP34 height, BIP68/113 locks, deployment-aware BIP141 witness commitment/unexpected-witness rules, mutated transaction Merkle trees, buried deployments, Taproot BIP9 state, Core-compatible configurable regtest Taproot activation, network-specific subsidy halving intervals, the 80,000 legacy/P2SH/witness sigop-cost limit, and Core-style transaction duplicate/null-input/base-size checks are enforced. Pinned Core 26 valid/invalid transaction pairs now cover representative BIP141/143 sighash and BIP147 activation behavior through rBTC's production adapter, with Core-backed BIP341 commitment and BIP342 tapscript execution smoke cases. Remaining gates include the full upstream vector corpus and keeping policy/standardness distinct from consensus.
- [ ] Differential tests against Bitcoin Core test vectors and `bitcoin-cli`/regtest. The first pinned Core 26 consensus vector pairs run in normal CI; full corpus ingestion, live `bitcoin-cli` comparison, property tests, and cargo-fuzz remain.
