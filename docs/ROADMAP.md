...
- [ ] Complete block/contextual validation: BIP30 is enforced with its two historical exceptions; BIP34 height, BIP68/113 locks, deployment-aware BIP141 witness commitment/unexpected-witness rules, mutated transaction Merkle trees, buried deployments, Taproot BIP9 state, Core-compatible configurable regtest Taproot activation, network-specific subsidy halving intervals, the 80,000 legacy/P2SH/witness sigop-cost limit, and Core-style transaction duplicate/null-input/base-size checks are enforced. The complete pinned Core 26 transaction-vector corpus now runs offline through rBTC's production script adapter: all 119 valid cases and all 70 invalid cases expressible by public consensus flags, with 9 `BADTX` structure cases and 14 policy-only cases classified separately. Constructed Core-backed cases additionally cover BIP341 commitments and BIP342 tapscript execution. Remaining gates include Core's full script corpus and keeping policy/standardness distinct from consensus.
- [ ] Differential tests against Bitcoin Core test vectors and `bitcoin-cli`/regtest. Core 26's complete `tx_valid.json`/`tx_invalid.json` corpus is pinned in normal CI with its public consensus-API boundary made explicit; full `script_tests.json` ingestion, live `bitcoin-cli` comparison, property tests, and cargo-fuzz remain.
