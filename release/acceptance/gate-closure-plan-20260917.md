...
1. Headers: replace node-owned history and serving copies with fallible disk
   ...
   are now identity-bound and used by startup/reindex validation. [A single record traversal](staged-single-pass-20260917.md) removes repeated batch verification. [Archive file writes and publication verification](archive-streamed-write-20260917.md) now stream compressed bytes; [initial partial-prefix publication](staged-prefix-streamed-publication-20260917.md) now streams source blocks; [already-published-prefix reconciliation](staged-prefix-streamed-comparison-20260917.md) now compares bounded streams byte-for-byte. [Archive truncation](ledger-truncation-streaming-20260917.md) now streams prefix rewrites through the durable recovery protocol; [Archive temporary admission failures](archive-resource-errors-20260917.md) retain their local-resource type at live node staging/publication boundaries; [typed reservation failures](reservation-error-kinds-20260918.md) now distinguish memory from execution-spool exhaustion through archive and execution errors without treating matching I/O messages as budget pressure; [unstaged memory retries](unstaged-memory-retry-20260918.md) now halve windows finitely with unchanged-tip, stage and script guards, while [complete unexecuted stages](staged-complete-reuse-20260918.md) can now survive startup and be revalidated/reused without rewriting; [staged checkpoints](staged-checkpoint-recovery-20260919.md) now preserve the full immutable stage across smaller batches and reconcile committed prefixes during normal/overlay startup; [staged memory retries](staged-memory-retry-20260919.md) now finitely halve batches with immutable-stage and unchanged-tip guards; live publication retries, fair resource waiting and broader post-commit auxiliary-index recovery remain open; [Piece hashing scratch](archive-piece-memory-20260917.md) now uses admitted 64 KiB buffers; [native file decoders](archive-decoder-memory-20260917.md) now retain linked-codec estimates plus Rust input allowance through decoding; [scan and visitor buffers](archive-scan-memory-20260917.md) now reserve before allocation; [returned archive payloads](archive-payload-ownership-20260917.md) now share immutable bytes and retain admission through node replay staging; [archive result arrays](archive-handle-memory-20260917.md) now share admitted storage through clones, merge overlap and partial iterators; [file manifest parsing and returned identities](archive-manifest-memory-20260917.md) now use bounded outputs and retained shared admission; [generated writer metadata](archive-write-manifest-memory-20260917.md) now admits fixed arrays/JSON before compression and retains returned identities; [native archive encoding](archive-encoder-memory-20260917.md) now admits native heap allocations, tracking storage and output buffers, including multithreaded workers; OS stacks, node metadata containers and legacy serving copies remain open. The user-selected default is now 32 GiB;
