# Default-redb admission resource acceptance

- Gate: `admission-resources`
- Commit: `f808a880e6d0536958f12b0f06b0fea153a62249`
- Source SHA-256: `aa12f0141ee8ec579f70fee9085c31bf895d949341415ef3e8e536cb2ece51f7`
- Acceptance checker SHA-256: `54c2ad722a738f6c676d2f5154c0eb8d628e27577e01e6a00a8f3bb1a207e73c`
- Functional test status: `PASS` (94 passed, 1 ignored; node admission tests 4 passed)
- Optimizer differential status: `PASS` (Core 31 ordering differential)
- Resource probe status: `PASS` (3 runs; peak clone RSS delta 204,800 bytes of 67,108,864; peak sampled RSS 49,094,656 bytes of 536,870,912)
- Recovery/fault status: `PASS`
- Acceptance status: `PASS`

Raw commands, logs, per-run probe JSON, Core adapter outputs and their SHA-256
values are listed in [the resource evidence manifest](resource-evidence-f808a88.sha256).
The raw directory is retained on the acceptance host at
`/data/bench/rbtc-resource-acceptance-f808c/`.
