...
type FlushWork = Box<dyn FnOnce() -> FlushResult + Send>;
...
impl<C: ExecutionChainStore + 'static> WriteBackChainstate<C> {
    ...
    pub fn take_last_flush(&self) -> Option<WriteBackFlush> {
        // Keep the buffered view visible until the engine outcome is known.
        let _pending = self.write();
    ...
    pub fn start_flush(&self) -> Result<bool, ChainStoreError> {
        self.start_flush_with_spawn(|work| {
    ...
    fn start_flush_with_spawn(
        &self,
        spawn: impl FnOnce(FlushWork) -> std::io::Result<JoinHandle<FlushResult>>,
    ) -> Result<bool, ChainStoreError> {
        self.check_failed()?;
        // Only one commit at a time: hold the mutation lock from settling
        // the previous commit through publishing its successor.
        let mut pending = self.write();
        self.check_failed()?;
        self.wait_in_flight_locked(Instant::now())?;
        // Publish pending -> in-flight while holding the same lock readers
        // acquire before capturing either layer. There must be no empty gap.
    ...
    fn wait_in_flight(
        ...
        let _pending = self.write();
        self.wait_in_flight_locked(waited_started)
    }

    // Caller holds the pending write lock until the outcome is published.
    fn wait_in_flight_locked(
        &self,
        waited_started: Instant,
    ) -> Result<Option<WriteBackFlush>, ChainStoreError> {
        let in_flight = self.lock_in_flight().take();
        in_flight
            .map(|flight| self.settle(flight, waited_started))
            .transpose()
    }
...
mod tests {
    ...
    #[test]
    fn failed_thread_spawn_keeps_the_buffer_readable_and_retryable() {
        let directory = TempDir::new().unwrap();
        let store = WriteBackChainstate::new(
            open(&directory, "buffered.redb"),
            WriteBackLimits {
        ...
        assert!(
            store
                .start_flush_with_spawn(|_| Err(std::io::Error::other("injected spawn failure")))
                .is_err()
        );
        assert_eq!(store.pending_blocks(), 1);
        assert_eq!(store.get(key(1)).unwrap(), Some(coin(5)));
        ...
        assert_eq!(store.inner().execution_tip().unwrap(), genesis);
        store.flush().unwrap();
        assert_eq!(store.inner().get(key(1)).unwrap(), Some(coin(5)));
        assert_eq!(store.inner().execution_tip().unwrap(), tip(1));
    }

    #[test]
    fn prefetch_is_refreshed_after_its_changes_leave_both_buffers() {
        let directory = TempDir::new().unwrap();
        let store = WriteBackChainstate::new(
            open(&directory, "buffered.redb"),
            WriteBackLimits::PASS_THROUGH,
        );
        let genesis = store.execution_tip().unwrap();
        store
            .commit_connect_batch(&[transition(genesis, tip(1), vec![], vec![(key(1), coin(5))])])
            .unwrap();
        store.flush().unwrap();
        let mut prefetched = store.get_many(&[key(1), key(2)]).unwrap();
        store
            .commit_connect_batch(&[transition(
                tip(1),
                tip(2),
            ...
            .unwrap();
        store.flush().unwrap();
        while store.take_last_flush().is_some() {}
        let mut durable_prefetch = prefetched.clone();
        store.reconcile_prefetch(&mut prefetched).unwrap();
        store
            .inner()
            .reconcile_prefetch(&mut durable_prefetch)
            .unwrap();
