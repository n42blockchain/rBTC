...
fn main() {
    cargo_print(&"rerun-if-changed=zstd/lib");
    assert!(
        !cfg!(feature = "pkg-config") && env::var_os("ZSTD_SYS_USE_PKG_CONFIG").is_none(),
        "rBTC requires vendored zstd allocation-failure fixes; system zstd is unsupported"
    );
