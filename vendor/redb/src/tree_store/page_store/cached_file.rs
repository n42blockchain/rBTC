...
impl PagedCachedFile {
    ...
    fn flush_write_buffer(&self) -> Result {
        ...
        const MAX_COALESCED_WRITE_BYTES: usize = 8 * 1024 * 1024;
        let mut run_offset: Option<u64> = None;
        let mut run = Vec::new();
        let flush_run = |run_offset: &mut Option<u64>, run: &mut Vec<u8>| -> Result {
            if let Some(offset) = run_offset.take() {
                self.file.write(offset, run)?;
                run.clear();
            }
            Ok(())
        };
        let mut writes = write_buffer
            .cache
            .iter()
            .map(|(offset, buffer)| (*offset, buffer))
            .collect::<Vec<_>>();
        writes.sort_unstable_by_key(|(offset, _)| *offset);
        for (offset, buffer) in writes {
            let buffer = buffer.as_ref().unwrap();
            let contiguous = run_offset.is_some_and(|start| {
                start
                    .checked_add(u64::try_from(run.len()).expect("write buffer length fits u64"))
                    == Some(offset)
            });
            if !contiguous
                || run
                    ...
                    .is_none_or(|length| length > MAX_COALESCED_WRITE_BYTES)
            {
                flush_run(&mut run_offset, &mut run)?;
            }
            if buffer.len() >= MAX_COALESCED_WRITE_BYTES {
                self.file.write(offset, buffer)?;
                continue;
            }
            if run_offset.is_none() {
                run_offset = Some(offset);
            }
