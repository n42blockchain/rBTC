    ...
    E: DatabaseKind,
{
    ...
    /// Writes a compacted copy of this environment to `destination`.
    ///
    /// A copy-on-write B-tree never reuses the pages a write transaction
    /// supersedes until they are recycled, so a long-running write workload
    /// accumulates pages that hold nothing. They still count against the
    /// geometry ceiling, because that is checked against the allocation
    /// high-water mark rather than against live content. This writes only the
    /// pages that are still reachable, which is what makes it useful to a
    /// caller running under a hard capacity limit.
    ///
    /// `destination` must not already exist. The copy is a complete, openable
    /// environment; this call does not disturb the source.
    ///
    /// Added to the vendored crate because upstream 0.6.6 exposes no binding
    /// for `mdbx_env_copy`, and its absence had been mistaken for MDBX
    /// lacking the capability. Keeping the `unsafe` here rather than at the
    /// call site is deliberate: the consuming crate forbids `unsafe_code`.
    ///
    /// # Errors
    ///
    /// Returns the MDBX error, including when `destination` already exists.
    pub fn copy_compact(&self, destination: &Path) -> Result<()> {
        let destination = destination
            .to_str()
            .ok_or(Error::Invalid)
            .and_then(|path| CString::new(path).map_err(|_| Error::Invalid))?;
        mdbx_result(unsafe {
            ffi::mdbx_env_copy(
                self.ptr().0,
                destination.as_ptr(),
