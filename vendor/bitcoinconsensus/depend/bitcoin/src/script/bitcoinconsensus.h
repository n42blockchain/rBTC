...
/// Verifies every input of one transaction while parsing the transaction and
/// constructing its shared signature-hash precomputation only once.
/// On script failure, failedInput receives the rejected input index.
EXPORT_SYMBOL int bitcoinconsensus_verify_transaction_with_spent_outputs(
                                    const unsigned char *txTo, unsigned int txToLen,
                                    const UTXO *spentOutputs, unsigned int spentOutputsLen,
