...
int bitcoinconsensus_verify_transaction_with_spent_outputs(
                                    const unsigned char *txTo, unsigned int txToLen,
                                    const UTXO *spentOutputs, unsigned int spentOutputsLen,
                                    unsigned int flags, unsigned int *failedInput,
                                    bitcoinconsensus_error* err)
{
    ...
    try {
        ...
        for (size_t i = 0; i < spentOutputsLen; i++) {
            CScript spk(spentOutputs[i].scriptPubKey,
                        spentOutputs[i].scriptPubKey + spentOutputs[i].scriptPubKeySize);
            spent_outputs.emplace_back(spentOutputs[i].value, std::move(spk));
        }

        set_error(err, bitcoinconsensus_ERR_OK);
        if (failedInput) {
            *failedInput = spentOutputsLen;
        }

        PrecomputedTransactionData txdata(tx);
        if (flags & bitcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT) {
            std::vector<CTxOut> txdata_outputs = spent_outputs;
            txdata.Init(tx, std::move(txdata_outputs));
        }

        for (unsigned int nIn = 0; nIn < spentOutputsLen; nIn++) {
            const CTxOut& spent = spent_outputs[nIn];
            if (!VerifyScript(tx.vin[nIn].scriptSig, spent.scriptPubKey,
                              &tx.vin[nIn].scriptWitness, flags,
                              TransactionSignatureChecker(&tx, nIn, spent.nValue, txdata,
