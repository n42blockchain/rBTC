...
/// Verifies every input while parsing and precomputing the transaction once.
///
/// The error contains the failing input index for a script failure. An absent
/// index denotes a transaction encoding, flag, or spent-output ABI failure.
pub fn verify_transaction_with_flags(
    ...
    unsafe {
        let mut error = Error::ERR_SCRIPT;
        let mut failed_input = spent_outputs.len() as c_uint;
        let ret = ffi::bitcoinconsensus_verify_transaction_with_spent_outputs(
            spending_transaction.as_ptr(),
            spending_transaction.len() as c_uint,
            spent_outputs.as_ptr() as *const c_uchar,
            spent_outputs.len() as c_uint,
        ...
        if ret == 1 {
            ...
            let input = (failed_input < spent_outputs.len() as c_uint)
                .then_some(failed_input as usize);
            Err((input, error))
        }
    }
}
...
pub mod ffi {
    ...
    extern "C" {
        ...
        /// Verifies every transaction input with one shared parse and
        /// signature-hash precomputation.
        pub fn bitcoinconsensus_verify_transaction_with_spent_outputs(
            tx_to: *const c_uchar,
            tx_tolen: c_uint,
            spent_outputs: *const c_uchar,
            num_spent_outputs: c_uint,
            flags: c_uint,
            failed_input: *mut c_uint,
            err: *mut Error,
        ) -> c_int;
    }
}
